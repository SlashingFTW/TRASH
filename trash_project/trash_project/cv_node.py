...
class CVNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('YOLOv12 CV Node starting...')

        # Publisher
        self.publisher_ = self.create_publisher(String, 'cv_direction', 10)
        #subscribe for cv_feedback
    ...
    def feedback_callback(self, msg):
        if msg.data == "done":
            self.get_logger().info("Received 'done' from motor_node. Ready for next object.")
            self.can_send = True
    ...
    def detect_loop(self):
        while rclpy.ok():
            ...
            # Run YOLO
            results = self.model(frame, conf=0.4, iou=0.9)[0]
            ...
            for box in results.boxes:
                ...
                # Only publish 'center' if allowed
                if third_w <= cx <= 2 * third_w and self.can_send:
                    msg = String()
                    msg.data = "center"
                    self.publisher_.publish(msg)
                    self.get_logger().info("Published: center")
                    self.can_send = False  # wait for feedback before sending again
            ...
            cv2.imshow("YOLOv12 Webcam Detection", results.plot())
